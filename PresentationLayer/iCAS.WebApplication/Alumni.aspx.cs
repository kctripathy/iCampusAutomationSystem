    ...
    public partial class Alumni: System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack && !IsCallback)
            {
                ...
                {
                    if ((Request.QueryString["Operation"].ToString().ToLower()) == "view")
                    {
                        Student_Multi.SetActiveView(View_Grid);
                    }
                    else if ((Request.QueryString["Operation"].ToString().ToLower()) == "approve")
                    {
                        Student_Multi.SetActiveView(View_Grid);
                    }
                    else if ((Request.QueryString["Operation"].ToString().ToLower()) == "detail")
                    {
                        Student_Multi.SetActiveView(View_Grid);
                    }
                    else
                ...
                else
                {
                    ...
                    BindDropdown_Gender();
                    SetValidationMessages();
                    //drpdwn_PresentDistrictId.SelectedIndex = 346;
                    //drpdwn_PresentDistrictId_SelectedIndexChanged(null, null);
                    Student_Multi.SetActiveView(InputControls);
                    //drpdwn_PresentDistrictId.SelectedIndex = 366;
                    //366
                }
            }
        ...
        protected void btn_Submit_Click(object sender, EventArgs e)
        {
            ...
            if (((Button)sender).Text.Trim().Equals(MicroEnums.DataOperation.Save.GetStringValue()))
            {
                ...
                if (ProcReturnValue > (int)MicroEnums.DataOperationResult.Success)
                {
                    ...
                    string mailSendingFlag = SendMailToUser();
                    lbl_TheMessage.Text = string.Format(@"<span color='green'><b>Registration done Successfully!</b>
                                                            <hr/>{0} email has sent to user for information.</span>", mailSendingFlag); // ReadXML.GetSuccessMessage("OK_STUDENT_ADDED");
                    dialog_Message.Show();
                ...
                {
                    dialog_Message.Show();
                    lbl_TheMessage.Text = "<span color='red'><b>Failed to register the supplied information</b></span>"; //ReadXML.GetFailureMessage("KO_STUDENT_ADDED");
                }
            }
            else
            {
        ...
        public string SendMailToUser()
        {
            ...
            else
            {
                ...
                string MailSubject = string.Format("<{0}> {1} Alumni Registration", MailSubjectPrefix, MailCompanyPrefix);
        ...
        private string GetHtmlTemplateCodeReadHTML()
        {
            ...
            string sFileName = Server.MapPath(".") + @"\App_Data\ICAS\html\HtmlPage_MailAlumniRegistration.html";
        ...
        void BindStudentProfile()
        {
            try
            {
                PageVariables.ThisStudent = (from xyz in StudentManagement.GetInstance.GetStudentList(false, false, true)
        ...
        protected void gridview_Students_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (!e.CommandName.Equals(MicroEnums.DataOperation.Page.GetStringValue()))
            {
                ...
                PageVariables.ThisStudent = (from xyz in StudentManagement.GetInstance.GetStudentList(true, false, true)
        ...
        {
            //int Ctr = 0;
            //string EstbStatus = gridview_Students.Text.ToString().Substring(0, 1);
            //for (int i = 0; i < gridview_Students.Rows.Count; i++)
            //{
            //    GridViewRow row = gridview_Students.Rows[i];
            ...
            //        int x = gridview_Students.GetInstance.UpdateEstablishmentStatus(EstbId, EstbStatus);
            //        Ctr++;
            //    }
            //}
        ...
        protected void drpdwn_PresentDistrictId_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                //District theDistrict = DistrictManagement.GetInstance.GetDistrictStateCountryByDistrictId(366); // 366 = ganjam
                txt_PresentStateName.Text = theDistrict.StateName;
                txt_PresentCountry.Text = theDistrict.CountryName;



                //drpdwn_PresentDistrictId.SelectedIndex = theDistrict.DistrictID;
                //drpdwn_PresentDistrictId.Text = theDistrict.DistrictName;
                //drpdwn_PresentDistrictId_SelectedIndexChanged(null, null);
            }
            catch
            {
        ...
        protected void gridview_Students_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            ...
            gridview_Students.DataSource = StudentManagement.GetInstance.GetStudentList(false, false, true);
            gridview_Students.DataBind();
        }

        protected void ddl_AcademicYear_SelectedIndexChanged(object sender, EventArgs e)
        {
            int fromYearIndex = int.Parse(ddl_AcademicYear.SelectedValue.ToString());

            List<AccountingYear> AccountingYearlist = AccountingYearManagement.GetInstance.GetAccountingYearList("");
            List<AccountingYear> PassingYearlist = (from i in AccountingYearlist
                                                    where i.AccountingYearID > fromYearIndex
                                                    select i).OrderBy(x => x.AccountingYearID).ToList(); //.ToList<AccountingYear>();
            ddl_PassingYear.DataSource = PassingYearlist;
            ddl_PassingYear.DataValueField = "AccountingYearID";
            ddl_PassingYear.DataTextField = "AccountingYearDescription";
            ddl_PassingYear.DataBind();
        ...
        protected void drpdwn_CourseId_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                objQuals = (from xyzl in QualManagement.GetInstance.GetQualsList()
                            where xyzl.ClassType.Contains("2,3")
                            select xyzl).ToList();
            }
            ddl_Qualification.Items.Clear();
            ddl_Qualification.DataSource = objQuals;
        ...
        public void ResetAll()
        {
            ...
            radio_LifeMember.ClearSelection();
        ...
        private void BindDetailViewStudent(int id)
        {
            ...
            //theStudent = student;
            theStudent = StudentManagement.GetInstance.GetStudentList(true, false, true).Find(a => a.StudentID == id);
            if (theStudent == null)
            {
        ...
        public void BindAccountingYear(string SearctText)
        {
            ...
            // passout year
            //AccountingYearlist = AccountingYearManagement.GetInstance.GetAccountingYearList(SearctText);
            ddl_PassingYear.DataSource = AccountingYearlist;
            ddl_PassingYear.DataValueField = "AccountingYearID";
            ddl_PassingYear.DataTextField = "AccountingYearDescription";
            ddl_PassingYear.DataBind();

            ddl_PassingYear.Items.RemoveAt(0); // remove the current session because of alumni can only be an ex student
            ddl_PassingYear.Enabled = true;
        }
        ...
        private int InsertStudent(List<StudentSubjectTaken> AllSubjects, List<StudentPreviousQual> StudentPreQualList)
        {
            ...
            TheStudent.RollNo = "";
            TheStudent.QualID = int.Parse(drpdwn_CourseId.SelectedValue == "--SELECT--" ? "-1" : drpdwn_CourseId.SelectedValue);
            TheStudent.StreamID = int.Parse(DropDown_StreamList.SelectedValue == "--SELECT--" ? "-1" : DropDown_StreamList.SelectedValue); //int.Parse(DropDown_StreamList.SelectedValue);
            TheStudent.ClassID = int.Parse(drpdwn_CourseId.SelectedValue == "--SELECT--" ? "-1" : drpdwn_CourseId.SelectedValue); //int.Parse(drpdwn_CourseId.SelectedValue);
            ...
            TheStudent.LifeMemberStatus = radio_LifeMember.SelectedValue;
            ...
            TheStudent.RegistrationNumber = txt_RegdNo.Text.ToString();
            TheStudent.AlumniPresentOccupation = txt_CurrentProfession.Text;
            TheStudent.AlumniYearOfPassing = ddl_PassingYear.Text;
            TheStudent.AlumniMembershipFeesPaid = 0;
            TheStudent.AlumniMembershipFeesPaidDetails = "fees not paid yet-";

            Returnvalue = StudentManagement.GetInstance.InsertStudent(TheStudent, AllSubjects, StudentPreQualList, true);
            return Returnvalue;
        }
        ...
        private int UpdateStudent()
        {
            ...
            PageVariables.ThisStudent.LifeMemberStatus = radio_LifeMember.SelectedValue;
            ...
            //PageVariables.ThisStudent.AlumniYearOfPassing = int.Parse(ddl_PassingYear.SelectedValue);
        ...
        private void PopulateFormField(Student theStudent)
        {
            try
            {
                ...
                ddl_AcademicYear.SelectedIndex = BasePage.GetDropDownSelectedIndex(ddl_AcademicYear, theStudent.SessionID);
                ddl_PassingYear.SelectedIndex = BasePage.GetDropDownSelectedIndex(ddl_PassingYear, theStudent.AlumniYearOfPassing);
                ...
                radio_LifeMember.SelectedIndex = BasePage.GetRadioButtonSelectedIndex(radio_LifeMember, theStudent.LifeMemberStatus);
                //PageVariables.ThisStudent.LifeMemberStatus = radio_LifeMember.SelectedValue;
