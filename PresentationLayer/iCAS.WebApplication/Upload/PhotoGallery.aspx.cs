...
namespace Micro.WebApplication.UPLOAD
{
    public partial class PhotoGallery : BasePage
    {
        ...
        protected void gridview_Establishment_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (!e.CommandName.Equals(MicroEnums.DataOperation.Page.GetStringValue()))
            {
                ...
                if (e.CommandName.Equals(MicroEnums.DataOperation.Select.GetStringValue()))
                {
                    ...
                    dialog_Message.Title = "Displaying Publication Record/Row:";
                    dialog_Message.Width = 400;
                    dialog_Message.Height = 400;
        ...
        private int InsertEstablishment()
        {
            ...
            objEstablishment.EstbMessage = Micro.Commons.Connection.LoggedOnUser.UserReferenceName;
        ...
        {
            string theNewFileName1 = string.Empty;
            string fileNameWithPath1 = string.Empty;

            string theNewFileName2 = string.Empty;
            string fileNameWithPath2 = string.Empty;
            ...
            if (fileUploadEstb.HasFile)
            {
                ...
                else
                {
                    ...
                    fileNameWithPath1 = Path.Combine(Server.MapPath("~/Documents"), theNewFileName1);
                    fileNameWithPath2 = Path.Combine(Server.MapPath("~/Documents"), theNewFileName2);

                    //save the file to our local path
                    fileUploadEstb.SaveAs(fileNameWithPath1);
                    fileUploadEstb.SaveAs(fileNameWithPath2);
