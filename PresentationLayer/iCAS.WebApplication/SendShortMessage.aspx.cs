...
{
    public partial class SendShortMessage : Page
    {
        ...
        protected void chk_Students_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                chkList_Classes.Visible = true;
                foreach (ListItem item in chkList_Classes.Items)
                {
                    item.Selected = true;
                }
            }
        ...
        protected void btnSendSMS_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < grdview_Students.Rows.Count; i++)
            {
                ...
                if (chkb.Checked)
                {
                    ...
                    string phoneNumber = row.Cells[3].Text.ToString();
                    if (phoneNumber.Trim().Equals(string.Empty) || phoneNumber.Trim().Equals("&nbsp;"))
                    {
                        continue;
                    }
                    ...
                    {
                        // SEND A SMS
                        string receipientName = row.Cells[2].Text.ToString();
                        string status = SendSMSNow(phoneNumber, receipientName);
                        if (status == "OK")
                        {
                            sendSuccess++;
        ...
        private List<Employee> GetStaffList()
        {
            ...
            lblStaffCount.Text = ((List<Employee>)(HttpRuntime.Cache[UniqueKey])).Count.ToString();
        ...
        {
            string UniqueKey = string.Concat("GetStudentList4SendingSMS");
            if (HttpRuntime.Cache[UniqueKey] == null)
            {
                ...
                List<Student> studentListWithValidPhoneNumbers = (from s in studentList
                                                                  where (!s.Mobile.Trim().Equals("") && s.Mobile.Length == 10)
                                                                  select s).ToList();

                HttpRuntime.Cache["studentListWithValidPhoneNumbers"] = studentListWithValidPhoneNumbers;
            }

            List<Student> theStudentList = (List<Student>)(HttpRuntime.Cache[UniqueKey]);
            List<Student> theFilteredStudent1 = new List<Student>();
            List<Student> theFilteredStudentFinal = new List<Student>();





            foreach (ListItem theItem in chkList_Classes.Items)
            {
                if (theItem.Selected == true)
                {
                    theFilteredStudent1 = (from ss in theStudentList
                                           where ss.ClassID.ToString() == theItem.Value
                                           select ss).ToList();

                    //theFilteredStudentFinal.Concat(theFilteredStudent1);
                    foreach (Student s in theFilteredStudent1)
                    {
                        theFilteredStudentFinal.Add(s);
                    }
                }
            }
            lblStudentsCount.Text = string.Format("{0}/{1}", theFilteredStudentFinal.Count, theStudentList.Count);
            //lblTextCount.Text = string.Format("- {0} ({1})", rblist_classes.SelectedItem.Text, theFilteredStudent1.Count);
            return theFilteredStudentFinal;
        ...
        private string SendSMSNow(string mobileNumber, string receipientName)
        {
            ...
            string strUrl = @"http://sandeshlive.in/API/WebSMS/Http/v1.0a/index.php?username=1tsdclg&password=maa@1234&sender=TSDCLG&to=" + mobileNumber + @"&message=Dear " + receipientName + ", " + txtMessage.Text + "&reqid=1&format={json|text}&route_id=23";
