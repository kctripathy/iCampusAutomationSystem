...
namespace Micro.IntegrationLayer.ICAS.LIBRARY
{
    public partial class LibraryIntegration
    {
        ...
        public static LibraryBook DataRowToLibraryBookModel(DataRow dRow)
        {
            ...
            lb.AccessionDate = dRow["AccessionDate"] is DBNull? DateTime.Parse("1/1/1000") : DateTime.Parse(dRow["AccessionDate"].ToString());
            lb.Title = dRow["Title"].ToString();
            lb.BookStatus = dRow["BookStatus"].ToString();
            lb.BookPrice = dRow["BookPrice"] is DBNull? 0:  float.Parse(dRow["BookPrice"].ToString());
            lb.AuthorID = dRow["AuthorID"] is DBNull ? 0 :  int.Parse(dRow["AuthorID"].ToString());
            lb.PublisherID = dRow["PublisherID"] is DBNull ? 0 :  int.Parse(dRow["PublisherID"].ToString());
            lb.SupplierID = dRow["SupplierID"] is DBNull ? 0 :  int.Parse(dRow["SupplierID"].ToString());
            lb.SegmentID = dRow["SegmentID"] is DBNull ? 0 :  int.Parse(dRow["SegmentID"].ToString());
            lb.CategoryID = dRow["CategoryID"] is DBNull ? 0 :  int.Parse(dRow["CategoryID"].ToString());
            ...
            lb.Pages = dRow["Pages"] is DBNull ? 0 : int.Parse(dRow["Pages"].ToString());
            lb.BookPrice = dRow["BookPrice"] is DBNull ? 0: float.Parse(dRow["BookPrice"].ToString());
            lb.BillNo = dRow["BillNo"].ToString();
            lb.BillDate = dRow["BillDate"] is DBNull ? DateTime.Parse("1/1/1000") : DateTime.Parse(dRow["BillDate"].ToString());
            lb.BookStatus = dRow["BookStatus"].ToString();
